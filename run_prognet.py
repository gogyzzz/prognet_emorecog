...
#        fz_fc1 = self.basicdnn_state_dict['net_state_dict']['fc1.
    ...
    def forward(self, x):
        ...
        ly1 = F.relu(F.dropout(self.fc1_w(x)))
        ly2 = F.relu(F.dropout(self.fc2_w(ly1)) + F.dropout(self.fc2_u(fzly1)))
        ly3 = F.relu(F.dropout(self.fc3_w(ly2)) + F.dropout(self.fc3_u(fzly2)))
        ly4 = F.relu(F.dropout(self.fc4_w(ly3)) + F.dropout(self.fc4_u(fzly3)))
...
def run_exp_case(exp_case):
    ...
    egemapsdataset = EgemapsDataset(exp_case['db_hdf'],exp_case['id_feat_dic_pk'], True)
    
    trainset = RCVEgemapsDataset(egemapsdataset, exp_case['run_fold_pk'], exp_case['irun'], exp_case['ifold'], 'train')

    traindataloader = DataLoader(trainset, batch_size=1024, shuffle=True)

    validset = RCVEgemapsDataset(egemapsdataset, exp_case['run_fold_pk'], exp_case['irun'], exp_case['ifold'], 'val')

    criterion = nn.CrossEntropyLoss()

    model = BasicDNN(exp_case['n_labels'],exp_case['pre_label'], True)

    optimizer = torch.optim.Adam(model.parameters())

    trainer = Trainer(exp_case['premodel_name'],traindataloader, validset, model, exp_case['pre_label'], 0, 600, optimizer, criterion, True)

    trainer.training()

    prev_state_dict = torch.load(exp_case['premodel_name'] + '.pth')

    model = ProgNet(prev_state_dict, 4, 'cat', True)

    optimizer = torch.optim.Adam(model.parameters())

    trainer = Trainer(exp_case['prognet_name'],traindataloader, validset, model, 'cat', 0, 600, optimizer, criterion, True)
